...
<div class="container top">
      ...
      <div class="page-header users-header">
        <h2>
          <?php echo ucfirst("Patterns");?>
          <a  href="<?php echo site_url("admin/site/add_pattern").'/'.$site['site_id'];?>" class="btn btn-success">Add New pattern</a>
          <a style="margin-right: 5px;"  href="<?php echo site_url("admin/site/scrap").'/'.$site['site_id'];?>" class="btn btn-success">Scrap Site</a>
        </h2>
      </div>
      ...
      ?>
            ...
            <tbody>
              <?php
              $i = 0;
              foreach($site_patterns as $row)
              {
                  $i++;
                  $status = ($row['pt_status']==1)?"Active":"InActive";
                echo '<tr>';
                echo '<td>'.$i.'</td>';
                echo '<td>'.ucfirst($row['pt_fld_name']).'</td>';
                echo '<td>'.$row['pt_fld_value'].'</td>';
                echo '<td>'.$status.'</td>';
                echo '<td>'.$row['pt_date'].'</td>';
                echo '<td class="crud-actions">
                  <a href="'.site_url("admin").'/site/update_pattern/'.$row['pt_id'].'/'.$row['site_id_FK'].'" class="btn btn-info">view & edit</a>  
                  <a href="'.site_url("admin").'/site/delete_pattern/'.$row['pt_id'].'/'.$row['site_id_FK'].'" class="btn btn-danger">delete</a>
                </td>';
                echo '</tr>';
              }
              ?>      
