...
//        $this->form_validation->set_rules('fld_pattern', 'Fields Url Required', 'trim|required');
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            $data_val['is_table'] = $this->input->post('is_table');

            $options = array();

            $options['total_colums'] = $this->input->post('total_colums');
            $options['req_colum'] = $this->input->post('req_colum');

            $data_val['pt_fld_options'] = json_encode($options);
...
//                }
                if ($is_table) {
                    $options_str = $pattern['pt_fld_options'];
                ...
                foreach ($html->find($pattern_str) as $div) {
                    ...
                    if ($is_table) {
                        ...
                        if ($i_option == $req_colum) {
                            ...
                            $flag_inner = true;
                            if(!empty($inner_pattern)){
                                foreach ($div->find($inner_pattern) as $div_inner) {
                                    $attr = $div_inner->getAllAttributes();
                                    if ($is_image)
                                        $img_src = $l_arr[$pattern_name] = $div_inner->src;
                                    if ($is_anchor)
                                        $href = $l_arr[$pattern_name] = $div_inner->href;
                                    if ($is_inner_text)
                                        $l_arr[$pattern_name] = $div_inner->plaintext; //innertext;
                                    if ($is_outer_text)
                                        $l_arr[$pattern_name] = $div_inner->outertext;
                                    $flag_inner = false;
                                }
                           }
                            ...
                            if (!empty($l_arr[$pattern_name]))
                                $l_arr['pattern_value'] = $l_arr[$pattern_name];
                            else
                                $l_arr['pattern_value'] = $div->plaintext;
                            $main_arr[$i][] = $l_arr;
                        }

                        if ($i_option == $total_colums) {
                            $i_option = 0;
                        }
                    } else {
                        $inner_pattern = '';
...
//echo $csv_output;exit;
        ...
        for ($k = 0; $k < sizeof($mainarr); $k++) {
            if($max_size<sizeof($mainarr[$k]))
                $max_size = sizeof($mainarr[$k]);
        }

        $j = 1;
        $flag = true;
        for ($k = 0; $k < sizeof($mainarr); $k++) {
            for ($k_i = 0; $k_i < $max_size; $k_i++) {
                $csv_output .= $j . $fieldseparator . " ";
                for ($j_k_i = 0; $j_k_i < sizeof($mainarr); $j_k_i++) {
                    if (isset($mainarr[$j_k_i][$k_i]['pattern_value'])) {
                        $csv_output .= str_replace($this->replace_arr, "", $mainarr[$j_k_i][$k_i]['pattern_value']) . $fieldseparator . " ";
                    } else {
                        $csv_output .= "NULL" . $fieldseparator . " ";
                    }
                }
