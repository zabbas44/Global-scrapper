...
//		CKEDITOR.replace( 'textarea_pop' );		
	...
	function generate_html(fl_id){
        ...
        function sendEmail(){
        	var html_resutl       = $('#html_gen').val();
		
		$('.alert').remove();
		$('.alert').remove();
		
		if(html_resutl == ''){
			var gen_alert = '<div class="alert alert-danger"><strong>Oh snap!</strong> Please provide URL to generate affiliate code And Email </div>';
			$('#url_pointer').append(gen_alert);
			return false;	
		}
                ...
                $.ajax({
                    url: $('#base_url').val()+"admin/send_html",
                    type: "POST",
                    context: document.body,
                    data:{html_gen:html_resutl,fld_ids:$("#fld_ids").val(),fld_urls:$("#fld_ids_url").val(),fld_urls_affiliate:$("#fld_ids_affiliate").val()},
                    cache: false,
                    success: function(data) {
                        
                        var gen_alert = '<div class="alert alert-success">Email Was Send to Your Account</div>';
                        $('#url_pointer').append(gen_alert);
                    }
